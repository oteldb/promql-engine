	...
	for stepIndex, vector := range vectors {
		...
		for i, stepBuckets := range o.seriesBuckets {
			...
			case "histogram_quantile":
				...
				v, forcedMonotonic, _, minBucket, maxBucket, maxDiff := promql.BucketQuantile(o.scalar1Points[stepIndex], stepBuckets)
				buf[n].AppendSample(uint64(i), v)
				if forcedMonotonic {
					warnings.AddToContext(annotations.NewHistogramQuantileForcedMonotonicityInfo(o.series[i].Get(labels.MetricName), posrange.PositionRange{}, vector.T, minBucket, maxBucket, maxDiff), ctx)
				}
			case "histogram_fraction":
