  ...
  + label_replace(metric, "new_label", "$1", "label", ".*")
	...
	for _, tcase := range cases {
		t.Run(tcase.name, func(t *testing.T) {
			ast, err := parser.NewParser(parser.Options{EnableExperimentalFunctions: true, ExperimentalDurationExpr: true, EnableExtendedRangeSelectors: true, EnableBinopFillModifiers: true}).ParseExpr(tcase.query)
			testutil.Ok(t, err)
...
func TestUnmarshalMatchers(t *testing.T) {
	...
	ast, err := parser.NewParser(parser.Options{EnableExperimentalFunctions: true, ExperimentalDurationExpr: true, EnableExtendedRangeSelectors: true, EnableBinopFillModifiers: true}).ParseExpr(expr)
	testutil.Ok(t, err)
