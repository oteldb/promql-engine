...
func TestPassthrough(t *testing.T) {
	expr, err := parser.NewParser(parser.Options{EnableExperimentalFunctions: true}).ParseExpr(`time()`)
	testutil.Ok(t, err)
	...
	t.Run("optimized with matching labels", func(t *testing.T) {
		selectorExpr, err := parser.NewParser(parser.Options{EnableExperimentalFunctions: true}).ParseExpr(`{region="east"}`)
		testutil.Ok(t, err)
	...
	t.Run("not optimized due to multiple engines", func(t *testing.T) {
		selectorExpr, err := parser.NewParser(parser.Options{EnableExperimentalFunctions: true}).ParseExpr(`{region=~"east|west"}`)
		testutil.Ok(t, err)
	...
	t.Run("optimized with matching labels on matrix selector", func(t *testing.T) {
		selectorExpr, err := parser.NewParser(parser.Options{EnableExperimentalFunctions: true}).ParseExpr(`{region="east"}[5m]`)
		testutil.Ok(t, err)
	...
	t.Run("optimized with multiple selectors matching the same engine", func(t *testing.T) {
		multiSelectorExpr, err := parser.NewParser(parser.Options{EnableExperimentalFunctions: true}).ParseExpr(`rate({region="east",__name__="metric_a"}[5m]) / {region="east",__name__="metric_b"}`)
		testutil.Ok(t, err)
	...
	t.Run("not optimized with selectors matching different engines", func(t *testing.T) {
		crossPartitionExpr, err := parser.NewParser(parser.Options{EnableExperimentalFunctions: true}).ParseExpr(`{region="east",__name__="metric_a"} / {region="west",__name__="metric_b"}`)
		testutil.Ok(t, err)
